...
int SManS::HandleAddService(int sfd, void const *buf, int len)
{
    char const *p;
    char const *name;
    Int32 size;
...
int SManS::HandleGetService(int sfd, void const *buf, int len)
{
    char const *name;
    ECode ec;
    InterfacePack ifcp;
    ...
    int _len = sizeof(int32_t) + sizeof(int32_t) + size;
    char _buf[_len];
...
int SManS::HandleRemoveService(int sfd, void const *buf, int len)
{
    char const *name;
    ECode ec;
...
int SManS::HandleReceiveEvent(int sfd, int events, void *user_data)
{
    int ret = 0;
    int type;
    void *buf;
...
int SManS::HandleConnectionEvent(int sfd, int events, void *user_data)
{
    int asfd;
    ...
    reed->fd = asfd;
    reed->handle = &SManS::HandleReceiveEvent;
    reed->user_data = NULL;
...
void *SManS::S_ServiceRoutine(void *arg)
{
    ECode ec = NOERROR;
    ...
    aee.events = EPOLLIN;
    aeed.fd = sfd;
    aeed.handle = &SManS::HandleConnectionEvent;
    aeed.user_data = NULL;
    aee.data.ptr = &aeed;
