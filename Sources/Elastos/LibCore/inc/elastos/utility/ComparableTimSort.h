...
private:
     ...
     * entry to the method.
     */
    CARAPI_(void) MergeCollapse();

    /**
     * Merges all runs on the stack until only one remains.  This method is
     * called once, to complete the sort.
     */
    CARAPI_(void) MergeForceCollapse();
...
//======================================================================================
template <typename T>
const Int32 ComparableTimSort<T>::MIN_MERGE = 32;

template <typename T>
const Int32 ComparableTimSort<T>::MIN_GALLOP = 7;

template <typename T>
const Int32 ComparableTimSort<T>::INITIAL_TMP_STORAGE_LENGTH = 256;

template <typename T>
const Boolean ComparableTimSort<T>::DEBUG = FALSE;

template <typename T>
ECode ComparableTimSort<T>::Sort(
    /* [in] */ ArrayOf<T *> * a)
{
    VALIDATE_NOT_NULL(a);
    ...
    /* [in] */ Int32 hi)
{
    ...
    if (lo < 0 || hi > a->GetLength()) {
        //ALOGE("ComparableTimSort<T>::Sort: E_ARRAY_INDEX_OUT_OF_BOUNDS_EXCEPTION: len: %d, start: %d, end: %d",
        //    a->GetLength(), lo, hi);
        return E_ARRAY_INDEX_OUT_OF_BOUNDS_EXCEPTION;
    }
    if (lo > hi) {
        //ALOGE("ComparableTimSort<T>::Sort: E_ILLEGAL_ARGUMENT_EXCEPTION: len: %d, start: %d, end: %d",
        //    a->GetLength(), lo, hi);
    ...
    , mStackSize(0)
{
    ...
    AutoPtr<ArrayOf<T *> > newArray = ArrayOf<T *>::Alloc(
        len < 2 * INITIAL_TMP_STORAGE_LENGTH ? len >> 1 : INITIAL_TMP_STORAGE_LENGTH);
    mTmp = newArray;
...
template <typename T>
void ComparableTimSort<T>::BinarySort(
    /* [in] */ ArrayOf<T *> * a,
    /* [in] */ Int32 lo,
    /* [in] */ Int32 hi,
    /* [in] */ Int32 start)
{
    ...
    for ( ; start < hi; start++) {
        ...
        while (left < right) {
            ...
            Int32 flagvalue = 0;
            if ((pivot->CompareTo(TO_IINTERFACE((*a)[mid]), &flagvalue), flagvalue) < 0) {
                right = mid;
            }
            else {
                left = mid + 1;
...
template <typename T>
Int32 ComparableTimSort<T>::CountRunAndMakeAscending(
    /* [in] */ ArrayOf<T *> * a,
    /* [in] */ Int32 lo,
    /* [in] */ Int32 hi)
{
    ...
    Int32 flagvalue = 0;
    if ((IComparable::Probe((*a)[runHi++])->CompareTo(TO_IINTERFACE((*a)[lo]), &flagvalue), flagvalue) < 0) { // Descending
        while(runHi < hi
            && (IComparable::Probe((*a)[runHi])->CompareTo((*a)[runHi - 1], &flagvalue), flagvalue) < 0) {
            runHi++;
        }
        ...
        while (runHi < hi
            && (IComparable::Probe((*a)[runHi])->CompareTo(TO_IINTERFACE((*a)[runHi - 1]), &flagvalue), flagvalue) >= 0) {
            runHi++;
        }
    }
...
template <typename T>
void ComparableTimSort<T>::ReverseRange(
    /* [in] */ ArrayOf<T *> * a,
    /* [in] */ Int32 lo,
    /* [in] */ Int32 hi)
{
...
template <typename T>
Int32 ComparableTimSort<T>::MinRunLength(
    /* [in] */ Int32 n)
{
    if (DEBUG) {
        assert(n >= 0);
...
template <typename T>
void ComparableTimSort<T>::PushRun(
    /* [in] */ Int32 runBase,
    /* [in] */ Int32 runLen)
{
...
template <typename T>
void ComparableTimSort<T>::MergeCollapse()
{
    while (mStackSize > 1) {
...
template <typename T>
void ComparableTimSort<T>::MergeForceCollapse()
{
    while (mStackSize > 1) {
        Int32 n = mStackSize - 2;
...
template <typename T>
void ComparableTimSort<T>::MergeAt(
    /* [in] */ Int32 i)
{
    if (DEBUG) {
        assert(mStackSize >= 2);
     ...
     */
    Int32 k = GallopRight(IComparable::Probe((*mA)[base2]), mA.Get(), base1, len1, 0);
    if (DEBUG) {
        assert(k >= 0);
    }
     ...
     */
    len2 = GallopLeft(IComparable::Probe((*mA)[base1 + len1 - 1]), mA.Get(), base2, len2, len2 - 1);
    if (DEBUG) {
        assert(len2 >= 0);
    }
...
template <typename T>
Int32 ComparableTimSort<T>::GallopLeft(
    /* [in] */ IComparable* key,
    /* [in] */ ArrayOf<T *> * a,
    /* [in] */ Int32 base,
    /* [in] */ Int32 len,
    /* [in] */ Int32 hint)
{
    ...
    Int32 ofs = 1;
    Int32 flagvalue = 0;
    if ((key->CompareTo(TO_IINTERFACE((*a)[base + hint]), &flagvalue), flagvalue) > 0) {
        // Gallop right until a[base+hint+lastOfs] < key <= a[base+hint+ofs]
        Int32 maxOfs = len - hint;
        while (ofs < maxOfs && (key->CompareTo(TO_IINTERFACE((*a)[base + hint + ofs]), &flagvalue), flagvalue) > 0) {
            lastOfs = ofs;
            ofs = (ofs << 1) + 1;
    ...
    while (lastOfs < ofs) {
        ...
        if ((key->CompareTo(TO_IINTERFACE((*a)[base + m]), &flagvalue), flagvalue) > 0) {
...
template <typename T>
Int32 ComparableTimSort<T>::GallopRight(
    /* [in] */ IComparable* key,
    /* [in] */ ArrayOf<T *> * a,
    /* [in] */ Int32 base,
    /* [in] */ Int32 len,
...
template <typename T>
void ComparableTimSort<T>::MergeLo(
    /* [in] */ Int32 base1,
    /* [in] */ Int32 len1,
    /* [in] */ Int32 base2,
    /* [in] */ Int32 len2)
{
    ...
    while (true) {
        ...
        do {
            ...
            Int32 flagvalue = 0;
            if (IComparable::Probe((*a)[cursor2])->CompareTo(TO_IINTERFACE((*tmp)[cursor1]), &flagvalue), flagvalue < 0) {
                a->Set(dest++, (*a)[cursor2++]);
                count2++;
                count1 = 0;
                if (--len2 == 0) {
        ...
        do {
            ...
            count1 = GallopRight(IComparable::Probe((*a)[cursor2]), tmp, cursor1, len1, 0);
            if (count1 != 0) {
            ...
            count2 = GallopLeft(IComparable::Probe((*tmp)[cursor1]), a, cursor2, len2, 0);
            if (count2 != 0) {
...
template <typename T>
void ComparableTimSort<T>::MergeHi(
    /* [in] */ Int32 base1,
    /* [in] */ Int32 len1,
    /* [in] */ Int32 base2,
    /* [in] */ Int32 len2)
{
    ...
    while (true) {
        ...
        do {
            ...
            Int32 flagvalue = 0;
            if (IComparable::Probe((*tmp)[cursor2])->CompareTo(TO_IINTERFACE((*a)[cursor1]), &flagvalue), flagvalue < 0) {
                a->Set(dest--, (*a)[cursor1--]);
                count1++;
                count2 = 0;
                if (--len1 == 0) {
        ...
        do {
            ...
            count1 = len1 - GallopRight(IComparable::Probe((*tmp)[cursor2]), a, base1, len1, len1 - 1);
            if (count1 != 0) {
                dest -= count1;
                cursor1 -= count1;
                len1 -= count1;
            ...
            count2 = len2 - GallopLeft(IComparable::Probe((*a)[cursor1]), tmp, 0, len2, len2 - 1);
            if (count2 != 0) {
                dest -= count2;
                cursor2 -= count2;
                len2 -= count2;
...
template <typename T>
AutoPtr<ArrayOf<T *> > ComparableTimSort<T>::EnsureCapacity(
    /* [in] */ Int32 minCapacity)
{
