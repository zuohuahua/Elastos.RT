...
void testAbsPositive()
{
    ...
    if (FAILED(ec)) {
        printf("testAbsPositive() Failed to create CBigInteger. Error %08X\n", ec);
        return;
    }
    ...
    for(int i = 0; i < resBytes->GetLength(); i++) {
        assertEquals("testAbsPositive() data error", (*resBytes)[i], (*rBytes)[i]);
    }

    Int32 sign;
    result->GetSignum(&sign);
    assertEquals("testAbsPositive() incorrect sign", 1, sign);
}
