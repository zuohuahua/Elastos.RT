...
static void ln_s(IFile *target, IFile *linkName);
static void ln_s(String target, String linkName);
...
#endif
static IFile *createTemporaryDirectory()
{
    AutoPtr<ISystem> system;
    CSystem::AcquireSingleton((ISystem**)&system);

    String base;
    system->GetProperty(String("java.io.tmpdir"), &base);
...
#endif
static String longString(int n)
{
...
#endif
static IFile *createDeepStructure(IFile *base)
{
    ECode ec;
    Boolean succeeded;
    String str;
    ...
    // ext has a limit of around 256 characters for each path entry.
    // 128 characters should be safe for everything but FAT.
    String lstr = longString(128);
    // Keep creating subdirectories until the path length is greater than 1KiB.
    // Ubuntu 8.04's kernel is happy up to about 4KiB.
    IFile *f = base;

    str = Object::ToString(f);
    len = str.GetLength();
...
static void test_longReadlink()
{
    ...
    Boolean b;
    source->Exists(&b);
    assertEquals("test_longReadlink", FALSE, b);

    target->Exists(&b);
    assertEquals("test_longReadlink", TRUE, b);

    String str;
    Int32 i;
    target->GetCanonicalPath(&str);
    i = str.GetLength();
    assertEquals("test_longReadlink", TRUE, i>1024);
}
...
#endif
static void test_emptyFilename()
{
    ECode ec;
    Boolean b;
    String str;
    ...
    ec = f->CreateNewFile(&b);
    if (FAILED(ec)) {
        printf("expected IOException, got it!\n");
    }
...
static void test_getCanonicalPath()
{
    ...
    target->GetCanonicalPath(&str1);
    linkName->GetCanonicalPath(&str2);
    ...
    str2 = NULL;
    target->GetCanonicalPath(&str1);
    linkName2->GetCanonicalPath(&str2);
    assertEquals("", str1, str2);
    ...
    str1 = NULL;
    str2 = NULL;
    longer->GetCanonicalPath(&str1);
    linkName3->GetCanonicalPath(&str2);
    assertEquals("", str1, str2);
...
static void ln_s(IFile *target, IFile *linkName)
{
    ...
    target->ToString(&str1);
    linkName->ToString(&str2);
    ln_s(str1, str2);
}
...
int mainFileTest(int argc, char *argv[])
{
    printf("\n==== libcore/math/FileTest ====\n");
    test_longReadlink();
    test_emptyFilename();
    test_getCanonicalPath();
    printf("\n==== end of libcore/math/FileTest ====\n");
