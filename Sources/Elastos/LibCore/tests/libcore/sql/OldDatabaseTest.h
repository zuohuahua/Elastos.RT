...
public:
    ...
    void testClose() {
        ...
        db->GetTable(String("test"),(ITableResult**)&outtable);
            // fail();
        // catch (Exception e) {
    ...
    void testExecStringCallback() {
        ...
        String outstr;
        IObject::Probe(res)->ToString(&outstr);
    ...
    void testExecStringCallbackStringArray() {
        ...
        (*args)[0] = String("table");
        db->Exec(String("select name from sqlite_master where type = '%q';"), (ICallback *)res.Get(), args);
        String outstr;
        IObject::Probe(res)->ToString(&outstr);
        ...
        ECode ec = db->Exec(String("select name from sqlite_master where type = "), (ICallback *)res.Get(), args);
        if (ec != NOERROR)
        {
...
//     }
    ...
    void testGetTableString() {
        ...
        AutoPtr<ITableResult> res;
        db->GetTable(String("select * from SIMPLE_TABLE1 ;"),(ITableResult**)&res);
        String emptystr;
        String resstr;
        IObject::Probe(emptyTable)->ToString(&emptystr);
        IObject::Probe(res)->ToString(&resstr);
...
//        assertFalse(emptyTable.toString().equals(res.toString()));
    ...
    void testGetTableStringStringArray() {
        ...
        // ECode ec = db->GetTable(String("select name from sqlite_master where type = "), *argsFail , NULL);
        // if (ec != NOERROR)
        // {
        //     printf("Testmethod should fail\n");
        // }
        AutoPtr<ITableResult> res;
        db->GetTable( String("select name from sqlite_master where type = '%q'"), args, (ITableResult**)&res);
        String outstr;
        IObject::Probe(res)->ToString(&outstr);
    ...
    void testGetTableStringStringArrayTableResult() {
        ...
        (*args)[0] = String("table");
        db->GetTable(String("select name from sqlite_master where type = '%q'"), argsFail, (ITableResult**)&res);
        String resstr;
        String defaultStr;
        IObject::Probe(res)->ToString(&resstr);
        IObject::Probe(defaultTableRes)->ToString(&defaultStr);
    ...
    void testVersion() {
        ...
        if (!version.IsNull()) {
            printf("versionsub==>%d\n", StringUtils::ParseInt32(version.Substring(0,1)));
            printf("version 1==>%s\n", version.string());
            dbhelper->Version(&version);
            printf("version 2==>%s\n", version.string());
        } else {
    ...
    //     db->Exec(String("insert into TEST values (") + StringUtils::ToString(input) + String(")"), NULL);
    //     AutoPtr<ITableResult> res;
    //     CTableResult::New((ITableResult**)&res);
    ...
    //     db->Exec(String("select sin(res) from TEST WHERE res = ") + StringUtils::ToString(input), (ICallback *)res.Get());
    //     String row;
    //     IObject::Probe(res)->ToString(&row);
    //     printf("row==>%s\n", row.string());
    ...
    //     db->GetTable(String("select sin(res) from TEST WHERE res = ")
    //                     + StringUtils::ToString(input),(ITableResult**)&res);
    ...
    void testSet_encoding() {
        ...
        String output;
        IObject::Probe(res)->ToString(&output);
        ...
        for (UInt32 i = 0; i < charsetNames->GetLength(); i++) {
            ...
            db->Exec(String("select * from encodingTest;"), ICallback::Probe(res));
            String encOutput;
            // String inputAsString = new String(encInputBytes,charsetNames[i]);
            IObject::Probe(res)->ToString(&encOutput);
            printf("encOutput: %s\n",encOutput.string());
        ...
        db->Exec(String("select * from encodingTest;"), ICallback::Probe(res));
        String encOutput1;
        IObject::Probe(res)->ToString(&encOutput1);
        printf("encOutput1: %s\n",encOutput1.string());
        ...
        db->Exec(String("select * from encodingTest;"), ICallback::Probe(res));
        String encOutput2;
        IObject::Probe(res)->ToString(&encOutput2);
        printf("encOutput2: %s\n",encOutput2.string());
...
//         // selects
//         db->GetTable(String("select * from private_table"), (ITableResult**)&resPriv);
//         db->GetTable(String("select * from public_table"), (ITableResult**)&resPub);
...
//     }
    ...
    void testCompileStringStringArray() {
        AutoPtr<ArrayOf<String> > args = ArrayOf<String>::Alloc(1);
        (*args)[0] = String("table");
        db->Compile(String("select name from sqlite_master where type = '%q';"), args, NULL);
    ...
    void testOpen_blob() {
        ...
        ECode ec = statement2->Bind(1, *in);
        if (ec != NOERROR)
        {
            PRINT_FILE_LINE_EX("Bind error:ec = %p",ec)
        }
        Boolean stepflag(FALSE);
    ...
    void testIs3() {
        ...
        ver = StringUtils::ParseInt32(verstr.Substring(0,1));
        Boolean is3flag(FALSE);
        db->Is3(&is3flag);
        if (is3flag) {
            printf("is3==>%d \n", ver == 3);
